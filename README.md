...
- **容量估算**：接水使用个人水杯容量；喝水时可选择水、奶茶、咖啡或饮料及默认容量，汇总按每条记录的容量估算；喝水次数不设上限。
...
当 `type` 为 `drink` 时，`drinkKind` 可选 `water`、`milkTea`、`coffee`、`beverage`，`volume` 为该杯饮品的毫升数；接水和上厕所记录可以传 `null`。旧版没有容量字段的喝水记录会按该成员水杯容量估算。
