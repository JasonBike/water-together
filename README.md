...
- **个人小档案**：登录时可快捷选择或自定义 100–2000 ml 水杯容量、性别和固定头像。
...
`name` 最多 12 个字符；`gender` 可选 `female`、`male`、`secret`；`cupCapacity` 为 100–2000 之间的整数（前端默认按 50 ml 调整）。同名成员会执行更新，不会重复创建。成功返回成员对象，参数不合法返回 `400`。
