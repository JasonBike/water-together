...
后端会根据日期从 SQLite 读取当天共享的匿名数字统计（准备杯数、喝水次数、上厕所次数、成员数和共同进度），再连同固定的写作要求发送给模型，不发送昵称或其他成员资料。小纸条会在当天首次打开、首次喝水/准备一杯、准备杯数跨过 4/5/8/10 杯等关键节点、重置当天记录，或点击“换一句”时更新；模型不可用时会自动回退到内置默认话术。若部署服务器上的模型服务地址不同，需要修改 `server.mjs` 顶部的固定地址和模型常量。
